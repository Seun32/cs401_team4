...
class CUBDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Trim unnecessary paths and load the image
        try:
            idx = img_path.split('/').index('CUB_200_2011')
            if self.image_dir != 'images':
                img_path = '/'.join([self.image_dir] + img_path.split('/')[idx+1:])
        ...
        if self.use_attr:
            ...
            if self.no_img:
                if self.n_class_attr == 3:
                    one_hot_attr_label = np.zeros((N_ATTRIBUTES, self.n_class_attr))
                    one_hot_attr_label[np.arange(N_ATTRIBUTES), attr_label] = 1
                    return one_hot_attr_label, class_label, img_path  # Return img_path
                else:
                    return attr_label, class_label, img_path  # Return img_path
            else:
                return img, class_label, attr_label, img_path  # Return img_path
        else:
            return img, class_label, img_path  # Return img_path
