...
flags.DEFINE_integer('train_epochs', 100, 'Number of training epochs.') #100
flags.DEFINE_integer('amortization_epochs', 10, 'Number of amortization epochs.') #10
...
flags.DEFINE_integer('n_groups', 112, 'Number of concept groups to use.')
...
flags.DEFINE_string('save_model_dir',
                    'saved_model',
                    'Directory to write the trained SavedModel.')
def main(_):
  train_set = tf_cub.get_dataset(train=True, augmented=True).take(100)
  train_set_unaugmented = tf_cub.get_dataset(train=True, augmented=False)
  test_set = tf_cub.get_dataset(train=False, augmented=False).take(100)
  ...
  if FLAGS.save_model_dir:
    ...
    tf.saved_model.save(model, FLAGS.save_model_dir)
    print(f"Model saved to: {FLAGS.save_model_dir}")
  ...
  for set_name in tqdm.tqdm(eval_sets):
